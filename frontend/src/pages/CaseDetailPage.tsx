...
export function CaseDetailPage() {
  ...
  return <main className="case-workbench">
    ...
    <div className="workbench-grid"><div className="workbench-main">
      ...
      <section className="side-card"><div className="side-card-title"><BookOpen size={17} /><h2>Evidence & citations</h2><strong>{item.evidence.length}</strong></div>{item.evidence.map((evidence) => <article className="evidence-item" key={evidence.chunk_id}><div><span className={`quality ${evidence.quality_status.toLowerCase()}`}>{evidence.quality_status}</span><small>{evidence.domain}</small></div><blockquote>“{evidence.citation_text}”</blockquote><strong>{evidence.document_title}</strong><span>{evidence.document_number} · {evidence.source_authority}</span><div className="citation-validation"><ShieldCheck size={12} />{evidence.validation}</div></article>)}</section>
      <section className="side-card hitl-card"><div className="side-card-title"><Flag size={17} /><h2>Đề xuất HITL</h2></div>{proposedActions.length ? proposedActions.map((action) => <div className="proposed-action" key={action}>{action.replaceAll('_', ' ')}</div>) : <div className="proposed-action success-action">READY FOR HUMAN REVIEW</div>}<button onClick={() => setHitlMessage('Đã chuyển hồ sơ tới chuyên viên phụ trách (mock).')} className="primary-button full-button">Chuyển chuyên viên rà soát</button>{hitlMessage && <p className="hitl-feedback"><Check size={13} />{hitlMessage}</p>}<p>Thao tác này chỉ mô phỏng handoff, không cập nhật quyết định tín dụng.</p></section>
    </aside></div>
  </main>
}
