...
export function DashboardPage() {
  ...
  return <main className="page readiness-dashboard">
    ...
    {todoCases.length > 0 && (
      <section className="card todo-cases-card" style={{ marginBottom: '16px', border: '1px solid #ff9a3f', background: '#fffaf5' }}>
        <div className="card-header" style={{ borderBottom: '1px solid #ffd2ae', background: '#fff3e8' }}>
          <div>
            <h2 style={{ color: '#c15c13', display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}>
              <Send size={15} /> Hồ sơ chờ duyệt / chuyển cấp của bạn ({todoCases.length})
            </h2>
            <p style={{ color: '#976135', margin: '4px 0 0' }}>Các hồ sơ đang ở cấp phê duyệt của bạn hoặc được chuyển tiếp lên.</p>
          </div>
        </div>
        <div className="table-scroll">
          <table className="readiness-table">
            <thead>
              <tr style={{ background: '#fff0e2' }}>
            ...
            <tbody>
              {todoCases.map((item) => (
                <tr key={item.id} style={{ background: '#fffcf9' }} onClick={() => window.location.href = `/cases/${item.id}`}>
                  <td>
                    <strong>{item.company_name}</strong>
                    <span>{item.id} · {item.context.customer_id}</span>
                  </td>
                  <td>
                    <ProductPill product={item.context.product} />
                  </td>
                  <td>
                    <strong>{new Intl.NumberFormat('vi-VN').format(item.context.requested_amount)} ₫</strong>
                  </td>
                  <td>
                    <span className="product-pill" style={{ background: '#ffeede', color: '#c15c13', textTransform: 'uppercase', fontSize: '9px', fontWeight: 'bold' }}>
                      {item.current_role === 'DIRECTOR' ? 'Giám đốc' : item.current_role === 'MANAGER' ? 'Quản lý' : 'Nhân viên'}
                    </span>
                  </td>
                  <td>
                    <FinalStatusPill status={item.workflow.final_status} approvalStatus={item.approval_status} currentRole={item.current_role} />
                  </td>
                  <td>
                    <Link className="row-open" to={`/cases/${item.id}`} style={{ background: '#fbead7', color: '#c15c13' }}>
                      <ArrowRight size={16} />
                    </Link>
                  </td>
                </tr>
